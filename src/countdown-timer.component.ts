...
export class CountdownTimer implements OnInit, OnDestroy {
    ...
    private getTimeDiff(datetime, useAsTimer = false) {
        ...
        if ( this.hour ) {
            ...
            timeString += this.twoDigit(hours) + ':';
        }

        if ( this.min ) {
            timeString += this.twoDigit(date_diff.getMinutes()) + ':';
        }

        if ( this.sec ) {
            timeString += this.twoDigit(date_diff.getSeconds());
        }

        if ( timeString ) {
            if ( timeString[timeString.length - 1] === ':' ) {
                timeString.slice(timeString.length - 1);
            }
